            ...
            try {
              ...
              return logEntries.filter(x => x && x.href != null).map(entry => withFallback(() => {
                return html`<tr>
                  ...
                  <td>${entry.ts ? formatDistance(entry.ts, now, { includeSeconds: true, addSuffix: true }) : ''}</td>
                  <td>
                    ${countriesByCode[entry.country]?.emoji ?? ''}
                    ${entry.visitor ? html`<img style="display:inline-block;margin-top:2px;" src="data:image/svg+xml;base64,${new Base64Encoder().encode(new TextEncoder().encode(picasso(entry.visitor.id)))}" width="16" height="16" alt="${entry.visitor.id}" />` : ''}
                  </td>
